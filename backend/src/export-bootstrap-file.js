/** Export bootstrap JSON from database (no server needed). */
const fs = require('fs');
...
function mapProvider(row) {
  return {
    ...
    imageUrl: row.image_url, avatarUrl: row.avatar_url, phone: row.phone || null, status: row.status,
...
async function exportBootstrap() {
  ...
  if (!user) throw new Error('Default user (id=1) not found');
...
module.exports = exportBootstrap;

if (require.main === module) {
  exportBootstrap().catch((e) => {
    console.error(e);
    process.exit(1);
  });
}
