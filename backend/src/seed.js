const { load, save, nextId: dbNextId } = require('./database');
...
async function seed() {
  ...
  const userId = await dbNextId(data, 'users');
  data.users.push({
    id: userId,
    name: 'Adaeze Chukwu',
  ...
  for (const p of providers) {
    const id = await dbNextId(data, 'providers');
  ...
  for (const s of [
    ...
    data.services.push({ id: await dbNextId(data, 'services'), provider_id: zaraId, ...s });
  }
  ...
  for (const b of [
    ...
    data.bookings.push({ id: await dbNextId(data, 'bookings'), created_at: new Date().toISOString(), ...b });
  }
  ...
  for (const p of [
    ...
    data.feed_posts.push({ id: await dbNextId(data, 'feed_posts'), created_at: new Date().toISOString(), ...p });
  }
  ...
  for (const n of [
    ...
    data.notifications.push({ id: await dbNextId(data, 'notifications'), read: 0, created_at: new Date().toISOString(), ...n });
  }
